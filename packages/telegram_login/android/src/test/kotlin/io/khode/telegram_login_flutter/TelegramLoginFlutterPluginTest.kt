...
/**
 * Unit tests for the pure [TelegramLoginFlutterPlugin.mapError] helper that
...
internal class TelegramLoginFlutterPluginTest {
    ...
    fun mapError_httpStatus_mapsToServerErrorWithParsedCode() {
        val (code, message) = TelegramLoginFlutterPlugin.mapError("HTTP 500: internal server error")
    ...
    fun mapError_httpFourHundred_isAlsoServerError() {
        val (code, message) = TelegramLoginFlutterPlugin.mapError("HTTP 404: not found")
        assertEquals("SERVER_ERROR", code)
    ...
    fun mapError_noAuthorizationCode_mapsToNoAuthCode() {
        val (code, _) = TelegramLoginFlutterPlugin.mapError("No authorization code in response URI")
        assertEquals("NO_AUTH_CODE", code)
    }

    @Test
    fun mapError_unknownHost_mapsToNetworkError() {
        val (code, _) = TelegramLoginFlutterPlugin.mapError("UnknownHostException: oauth.telegram.org")
        assertEquals("NETWORK_ERROR", code)
    }

    @Test
    fun mapError_socketTimeout_mapsToNetworkError() {
        val (code, _) = TelegramLoginFlutterPlugin.mapError("SocketTimeoutException: connect timed out")
        assertEquals("NETWORK_ERROR", code)
    }

    @Test
    fun mapError_genericNetworkError_mapsToNetworkError() {
        val (code, message) = TelegramLoginFlutterPlugin.mapError("Network error")
        assertEquals("NETWORK_ERROR", code)
    ...
    fun mapError_unknownMessage_fallsBackToRequestFailed() {
        val (code, message) = TelegramLoginFlutterPlugin.mapError("Something weird happened")
    ...
    fun mapError_nullMessage_fallsBackToRequestFailed() {
        val (code, message) = TelegramLoginFlutterPlugin.mapError(null)
        assertEquals("REQUEST_FAILED", code)
